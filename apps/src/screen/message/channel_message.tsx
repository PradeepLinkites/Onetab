...
import { Back, BackIcon } from "../../assets";
...
export const ChannelMessage = (props: any) => {
  ...
  useEffect(() => {
    ...
    // dispatch(
    //   getChatAccessToken({
    //     matrixUsername: "yogesh_3tcrgxtl",
    //     matrixPassword: "mum1ura6j",
    //   })
    // );
    // setTimeout(() => {
    //   datasetter();
  ...
  const createArrayFromTimeline = (timeline: any[]) => {
    ...
    if (timelineItem.getType() === "m.room.member") {
      ...
      if (threadItem["m.relates_to"] !== undefined) {
        const reactionItemIndex = msgs.findIndex(
          (x: { eventId: any }) =>
            x.eventId === threadItem["m.relates_to"].event_id
        );
        var msgType = "text";
        if (
      ...
      const reductionItemIndex = msgs.findIndex(
        (x: { eventId: any }) => x.eventId === timelineItem.getAssociatedId()
      );

      if (reductionItemIndex !== -1) {
        msgs.splice(reductionItemIndex, 1);
      } else {
        msgs.forEach((element: { emojiArray: any[]; thread: any[] }) => {
          const emojiIndex = element.emojiArray.findIndex(
            (x: { eventId: any }) =>
              x.eventId === timelineItem.getAssociatedId()
          );
          if (emojiIndex !== -1) {
  ...
  useEffect(() => {
    if (currentEvent !== undefined) {
      ...
      const checkNewItem = msgs.filter(
        (item: { eventId: any }) => item.eventId === currentEvent.event.getId()
      );
  ...
  const splitData = (input: any[], isScroll: boolean) => {
    const sortList = input.sort(
      (a: { date: number }, b: { date: number }) => a.date - b.date
    );

    const result = sortList.reduce(
      (accum: any[], current: { date: { toLocaleDateString: () => any } }) => {
        let dateGroup = accum.find(
          (x: { title: { toLocaleDateString: () => any } }) =>
    ...
    if (selectedEmoji !== null && selectedEmoji !== undefined) {
      if (event === undefined) {
        ...
        const emojiIndex = event.emojiArray.findIndex(
          (x: { item: any }) => x.item === selectedEmoji
        );
  ...
  const matrixQualifiedUserId = (user_id: string) => {
  ...
  const updatedMessage = (text: string | string[]) => {
  ...
  // const checkUser = (text: string | string[]) => {
  //   checkUrls(text);
  //   if (text.indexOf("@") !== -1) {
  //     var newText = text;
  ...
  const insertAt = (mainString: string, index: number, subString: string) => {
    if (index > 0) {
      return (
  ...
  const checkUrls = (inputText: string) => {
    if (
  ...
  const msgSlot = (date: moment.MomentInput) => {
    ...
    <SafeAreaView style={styles.mainContainer}>
      ...
      <View style={styles.container}>
        ...
        <View>
          ...
          >
            <Text style={styles.headerDetailView}>See detail here</Text>
          </Pressable>
